# DREADLAND - Credits & Attribution

## Development

**Game Design & Programming:** [Your Name]

**Conflict Resolution Systems Lead:** [Daughter, Age 9]
*"Resolving conflicts peacefully"*

---

## Third-Party Assets

This game uses assets from the **Liberated Pixel Cup (LPC)** community. All LPC assets are used under their respective open-source licenses.

### Licenses Used

- **OGA-BY 3.0** - OpenGameArt.org Attribution License 3.0
- **CC-BY-SA 3.0** - Creative Commons Attribution-ShareAlike 3.0
- **CC-BY-SA 4.0** - Creative Commons Attribution-ShareAlike 4.0
- **GPL 3.0** - GNU General Public License 3.0
- **CC-BY** - Creative Commons Attribution

---

## Character Sprites (LPC)

### Base Bodies & Heads

**Female Body Base**
- Authors: Benjamin K. Smith (BenCreating), bluecarrot16, TheraHedwig, Evert, MuffinElZangano, Durrani, Pierre Vigier (pvigier), ElizaWy, Matthew Krohn (makrohn), Johannes Sjölund (wulax), Stephen Challener (Redshrike)
- Licenses: OGA-BY 3.0, CC-BY-SA 3.0, GPL 3.0
- Sources:
  - https://opengameart.org/content/liberated-pixel-cup-lpc-base-assets-sprites-map-tiles
  - https://opengameart.org/content/lpc-medieval-fantasy-character-sprites
  - https://opengameart.org/content/lpc-ladies
  - https://opengameart.org/content/lpc-character-bases

**Female Head**
- Authors: ElizaWy, Stephen Challener (Redshrike)
- Licenses: OGA-BY 3.0, CC-BY
- Note: Original head by Redshrike, reduced size head by ElizaWy

### Hair

**Female Hair (Braid)**
- Authors: Yamilian, bluecarrot16
- Licenses: CC-BY-SA 3.0, GPL 3.0
- Sources:
  - https://opengameart.org/content/lpc-heroine
  - https://opengameart.org/content/lpc-hair

**Male Hair (Buzzcut)**
- Authors: ElizaWy
- Licenses: OGA-BY 3.0
- Source: https://opengameart.org/content/lpc-hair

### Armor (Copper Plate Set)

**Helmet (Bascinet)**
- Authors: bluecarrot16, ElizaWy, Sander Frenken (castelonia)
- Licenses: OGA-BY 3.0, CC-BY-SA 4.0, CC-BY-SA 3.0, GPL 3.0
- Sources:
  - https://opengameart.org/content/lpc-realistic-helmet-pack
  - https://opengameart.org/content/lpc-helmets-mega-pack
  - https://opengameart.org/content/lpc-expanded-hats-facial-helmets

**Shoulders, Arms, Torso, Legs, Feet (Plate Armor)**
- Authors: JaidynReiman, bluecarrot16, Michael Whitlock (bigbeargames), Matthew Krohn (makrohn), Johannes Sjölund (wulax)
- Licenses: OGA-BY 3.0, CC-BY-SA 3.0, GPL 3.0
- Note: Original by wulax, adapted to female base by makrohn, recolors by bigbeargames, adapted to v3 bases by bluecarrot16
- Sources:
  - https://opengameart.org/content/lpc-medieval-fantasy-character-sprites
  - https://opengameart.org/content/lpc-combat-armor-for-women
  - http://opengameart.org/content/lpc-clothing-updates

**Gloves**
- Authors: Michael Whitlock (bigbeargames), Matthew Krohn (makrohn), Johannes Sjölund (wulax), bluecarrot16, JaidynReiman
- Licenses: OGA-BY 3.0, CC-BY-SA 3.0, GPL 3.0
- Sources:
  - https://opengameart.org/content/lpc-medieval-fantasy-character-sprites
  - http://opengameart.org/content/lpc-clothing-updates

---

## Enemy Sprites

### Skeleton Guardian

**Skeleton Body & Head**
- Authors: bluecarrot16, Johannes Sjölund (wulax), Stephen Challener (Redshrike)
- Licenses: OGA-BY 3.0, CC-BY-SA 3.0, GPL 3.0
- Sources:
  - https://opengameart.org/content/liberated-pixel-cup-lpc-base-assets-sprites-map-tiles
  - https://opengameart.org/content/lpc-medieval-fantasy-character-sprites
  - https://opengameart.org/content/lpc-skeleton
  - https://opengameart.org/content/lpc-character-bases

**Skeleton Eyes**
- Authors: JaidynReiman, Matthew Krohn (makrohn), Stephen Challener (Redshrike)
- Licenses: OGA-BY 3.0, CC-BY-SA 3.0, GPL 3.0
- Note: Original by Redshrike, mapped to all frames by Matthew Krohn & JaidynReiman
- Source: https://opengameart.org/content/liberated-pixel-cup-lpc-base-assets-sprites-map-tiles

---

## Weapons

### Longsword
- Authors: Johannes Sjölund (wulax), bluecarrot16
- Licenses: OGA-BY 3.0, CC-BY-SA 3.0
- Sources:
  - https://opengameart.org/content/lpc-medieval-fantasy-character-sprites
  - https://opengameart.org/content/lpc-extended-weapon-animations

---

## Tools

### Axe & Pickaxe
- Authors: bluecarrot16, Pierre Vigier (pvigier), Tuomo Untinen (reemax), Johannes Sjölund (wulax), Inboxninja
- Licenses: CC-BY-SA 3.0
- Source: https://opengameart.org/content/lpc-hand-tools

### Facial Expressions
- Authors: JaidynReiman, ElizaWy, Stephen Challener (Redshrike)
- Licenses: OGA-BY 3.0
- Note: Original by Redshrike, expressions by ElizaWy, mapped to all frames by JaidynReiman
- Sources:
  - https://github.com/ElizaWy/LPC/tree/main/Characters/Head
  - https://opengameart.org/content/ulpc-expanded-expressions

---

## Sound Effects

Sound effects are used under their respective licenses. See individual asset folders for specific attribution.

---

## Special Thanks

- **Godot Engine** - https://godotengine.org
- **OpenGameArt.org** - For hosting the LPC community assets
- **Liberated Pixel Cup** - The amazing community of pixel artists
- All playtesters and early supporters

---

## LPC Universal Spritesheet Generator

Character sprites were assembled using the LPC Universal Spritesheet Generator tools.
- https://sanderfrenken.github.io/Universal-LPC-Spritesheet-Character-Generator/

---

## Contact

For attribution questions or concerns, please contact [Your Email].

---

*Last updated: 2025-12-27*
