extends Node
## AshbaneCosmetics - Maps Ashbane achievements to in-game cosmetics
...
# ===============================================================================
# TIER-BASED COSMETICS (Automatic based on Ashbane tier)
# ===============================================================================
...
# ===============================================================================
# LPC LAYER MAPPING
# Maps Ashbane cosmetics to LPC sprite layers
# ===============================================================================
...
func _ready() -> void:
	# Connect to AshbaneAuth profile updates
	if AshbaneAuth:
...
func calculate_cosmetics(profile: Dictionary) -> Dictionary:
	"""Calculate all cosmetics a player should have based on their Ashbane profile"""
	var result = {
		"tier": "initiate",
		"armor_set": "",
		"effects": [],
	...
	var ashbane = profile.get("ashbane", {})
	var tier = ashbane.get("tier", "initiate").to_lower()
...
func apply_to_character(character: Node, cosmetics: Dictionary, instant: bool = false) -> void:
	...
	if not sprite:
		LogManager.warning("No CharacterSprite found on character", "ashbane")
		return
...
func _apply_cosmetics_instant(character: Node, sprite: Node, cosmetics: Dictionary) -> void:
	...
	if TIER_ARMOR_PATHS.has(tier):
		...
		if character.has_method("set_ashbane_cosmetics"):
			character.set_ashbane_cosmetics(cosmetics)
...
func _apply_glow_effect(character: Node, color: Color) -> void:
	...
	var existing_glow = character.get_node_or_null("AshbaneGlow")
	if existing_glow:
...
func _on_profile_updated(profile: Dictionary) -> void:
	"""Handle profile update from AshbaneAuth"""
	var cosmetics = calculate_cosmetics(profile)
	LogManager.info("Calculated cosmetics for tier: %s" % cosmetics.get("tier", "unknown"), "ashbane")
	cosmetics_calculated.emit(cosmetics)
