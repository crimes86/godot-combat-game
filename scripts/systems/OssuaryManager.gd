...
const INFLUENCE_TIER_NAMES: Array[String] = ["Unnoticed", "Noticed", "Hunted", "Marked"]
...
var _influence_panel_style: StyleBoxFlat = null
var _influence_panel: PanelContainer = null
...
func _create_hud() -> void:
	...
	_hud_container.set_anchors_preset(Control.PRESET_CENTER_TOP)
	_hud_container.offset_left = -90
	_hud_container.offset_right = 90
	_hud_container.offset_top = 10
	_hud_container.offset_bottom = 50
	_hud_layer.add_child(_hud_container)

	# Background panel — matches corruption meter styling
	var panel = PanelContainer.new()
	panel.name = "BgPanel"

	_influence_panel_style = StyleBoxFlat.new()
	_influence_panel_style.bg_color = Color(0.05, 0.05, 0.08, 0.7)
	_influence_panel_style.corner_radius_top_left = 4
	_influence_panel_style.corner_radius_top_right = 4
	_influence_panel_style.corner_radius_bottom_left = 4
	_influence_panel_style.corner_radius_bottom_right = 4
	_influence_panel_style.content_margin_left = 8
	_influence_panel_style.content_margin_right = 8
	_influence_panel_style.content_margin_top = 5
	_influence_panel_style.content_margin_bottom = 5
	_influence_panel_style.border_width_top = 1
	_influence_panel_style.border_width_bottom = 1
	_influence_panel_style.border_width_left = 1
	_influence_panel_style.border_width_right = 1
	_influence_panel_style.border_color = Color(0.3, 0.4, 0.3, 0.4)
	panel.add_theme_stylebox_override("panel", _influence_panel_style)
	panel.mouse_filter = Control.MOUSE_FILTER_STOP
	_influence_panel = panel
	_hud_container.add_child(panel)

	# --- Influence row (bar + tier label) ---
	var influence_row = HBoxContainer.new()
	influence_row.add_theme_constant_override("separation", 8)
...
func _create_bar(initial_value: float) -> ProgressBar:
	"""Create a styled progress bar matching corruption meter bar."""
	...
	bar_fill.bg_color = Color(0.3, 0.55, 0.35)  # Default quiet green, updated dynamically
...
func _update_hud() -> void:
	...
	if _influence_panel_style:
		_influence_panel_style.border_color = inf_border_color

	# Update tooltip
	if _influence_panel:
		var tier_name = get_influence_tier_name()
		var next_threshold := ""
		if _local_influence < 25.0:
			next_threshold = "Next tier at 25"
		elif _local_influence < 50.0:
			next_threshold = "Next tier at 50"
		elif _local_influence < 75.0:
