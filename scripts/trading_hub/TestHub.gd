...
func _ready() -> void:
	...
	if profiler_scene:
		...
		add_child(profiler)
		print("[TestHub] Performance profiler added (F3 to toggle)")
...
func _spawn_wall_torches() -> void:
	...
	All positions match exact collision polygon vertices from TestHub.tscn
	"""
...
func _setup_time_lighting() -> void:
	...
	if _canvas_modulate and has_node("/root/TimeManager"):
		...
		print("[TestHub] Time sync enabled - cave lighting active")
	else:
		# Fallback to static cave lighting if no TimeManager
		if _canvas_modulate:
			_canvas_modulate.color = Color(0.6, 0.58, 0.55, 1.0)
		print("[TestHub] TimeManager not found - using static cave lighting")
...
func _create_north_entrance_light() -> void:
	...
	print("[TestHub] North entrance daylight bleed created")
...
func _create_zone2_sunlight() -> void:
	...
	print("[TestHub] Zone 2 outdoor sunlight created")
...
func _update_zone2_sunlight(hour: float) -> void:
	...
	print("[TestHub] Zone2 sunlight updated: hour=%.1f energy=%.2f" % [hour, intensity])
...
func _spawn_grassland_details() -> void:
	...
	print("[TestHub] Spawned grassland details: %d children" % grass_container.get_child_count())
...
func _create_horizon_atmosphere() -> void:
	...
	print("[TestHub] Created horizon atmosphere effects")
...
func _create_edge_softening(_grass_edge_vertices: PackedVector2Array) -> void:
	...
	print("[TestHub] Created edge softening effects")
...
func _spawn_cliff_debris(container: Node2D) -> void:
	...
	# Use EXACT collision polygon vertices from TestHub.tscn
...
func _create_grass_transition_patches(container: Node2D, jagged_vertices: PackedVector2Array) -> void:
	...
	print("[TestHub] Creating grass transition patches along %d jagged vertices" % jagged_vertices.size())
...
func _spawn_zone2_trees() -> void:
	...
	print("[TestHub] Initializing Zone 2 tree system (proximity-based)...")
	...
	for path in ZONE2_TREE_TEXTURES:
		...
		if tex:
			...
			print("[TestHub] WARNING: Failed to load tree texture: %s" % path)

	if _zone2_tree_textures_loaded.is_empty():
		print("[TestHub] ERROR: No tree textures loaded!")
		return
	...
	print("[TestHub] Zone 2 tree data generated: %d trees (0 nodes until player nearby)" % _zone2_tree_data.size())
