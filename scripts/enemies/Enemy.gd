...
var _weakpoint_index: int = 0        # Next config to spawn
...
func get_crit_window_weakpoint_count() -> int:
	"""Returns the number of weakpoints for this crit window (influence-based)."""
	if weakpoints.size() > 0:
		return weakpoints.size()
	# Influence tier determines weakpoint count: tier 0=1, tier 1=2, tier 2=3, tier 3=4
	if OssuaryManager:
...
func spawn_weakpoints() -> void:
	...
	_weakpoint_configs.clear()
	_weakpoint_index = 0

	# Calculate weakpoint count based on influence tier
	...
	# SERVER: spawn all weakpoints at once (needed for validation count)
	if _is_server_mode:
		for config in _weakpoint_configs:
			_spawn_single_weakpoint(config)
		skeleton_crit_windows_triggered += 1
		return

	# CLIENT: spawn only the first weakpoint (sequential)
	if _weakpoint_configs.size() > 0:
		_weakpoint_index = 0
		_spawn_single_weakpoint(_weakpoint_configs[0])

	# Tutorial: Show red arrow pointing at first weakpoint for the first N crit windows
	skeleton_crit_windows_triggered += 1
	if skeleton_crit_windows_triggered <= TUTORIAL_ARROW_CRIT_WINDOWS and _weakpoint_configs.size() > 0:
		create_and_show_weakpoint_arrow(_weakpoint_configs[0]["position"])

func _spawn_single_weakpoint(config: Dictionary, do_grow_in: bool = false) -> void:
...
func _on_weakpoint_destroyed_local(weakpoint) -> void:
	"""Handle weakpoint destruction: advance to next sequential weakpoint."""
	_weakpoint_index += 1

	# Spawn next weakpoint BEFORE forwarding destroy signal so CritWindowManager
	# sees the new spawned count before checking completion
	if not _is_server_mode and _weakpoint_index < _weakpoint_configs.size():
		_spawn_single_weakpoint(_weakpoint_configs[_weakpoint_index], true)
